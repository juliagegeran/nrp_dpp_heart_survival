...
path1 <- "C:/Users/julia/SRP/S&D/dataset/pubsaf2309/"
path2 <- "C:/Users/julia/SRP/S&D/dataset/pubsaf2303/"

cand_thor <- read_sas(paste0(path1, "cand_thor.sas7bdat"), NULL) %>%  
  zap_formats() %>% zap_labels()

tx_hr <- read_sas(paste0(path1, "tx_hr.sas7bdat"), NULL) %>%  
  zap_formats() %>% zap_labels()

txf_hr <- read_sas(paste0(path1, "txf_hr.sas7bdat"), NULL) %>%  
   zap_formats() %>% zap_labels()

just_form_hr <- read_sas(paste0(path2, "JustFormHR.sas7bdat"), NULL) %>%  
  zap_formats() %>% zap_labels()

just_form_hr_stat1 <- 
  read_sas(paste0(path2, "JustFormHRStat1.sas7bdat"), NULL) %>%  
  zap_formats() %>% zap_labels()

just_form_hr_stat2 <- 
  read_sas(paste0(path2, "JustFormHRStat2.sas7bdat"), NULL) %>%  
  zap_formats() %>% zap_labels()

just_form_hr_stat3 <- 
  read_sas(paste0(path2, "JustFormHRStat3.sas7bdat"), NULL) %>%  
  zap_formats() %>% zap_labels()

just_form_hr_stat4 <- 
  read_sas(paste0(path2, "JustFormHRStat4.sas7bdat"), NULL) %>%  
  zap_formats() %>% zap_labels()

risk_strat_data_hr <- 
  read_sas(paste0(path2, "RiskStratDataHR.sas7bdat"), NULL) %>%  
  zap_formats() %>% zap_labels()

just_form_hr_data_link <- 
  read_sas(paste0(path2, "JustFormHRDataLink.sas7bdat"), NULL) %>%  
  zap_formats() %>% zap_labels()

status_just_episode <- 
  read_sas(paste0(path2,"StatusJustEpisode.sas7bdat"), NULL) %>%
  zap_formats() %>% zap_labels()

institution <- read_sas(paste0(path2, "institution.sas7bdat"), NULL) %>%
  zap_formats() %>% zap_labels()

opo <- read_sas(paste0(path2, "hist_opo_txc.sas7bdat"), NULL) %>%
  zap_formats() %>% zap_labels()

donor <- read_sas(paste0(path2, "donor_deceased.sas7bdat"), NULL) %>%
  zap_formats() %>%  zap_labels()

deceasedtodonhosp <- read_sas(paste0(path2, "deceasedtodonhosp.sas7bdat"), NULL) %>% 
  zap_formats() %>% zap_labels()

donorhospital <- read_sas(paste0(path2, "donorhospital.sas7bdat"), NULL) %>% 
  zap_formats() %>% zap_labels()

donor_disposition <- read_sas(paste0(path2, "donor_disposition.sas7bdat"), NULL) %>% 
  zap_formats() %>% zap_labels()
...
dondeathdatetime <- read_sas(paste0(path2, "dondeathdatetime.sas7bdat"), NULL) %>% 
  zap_formats() %>% zap_labels()
...
cand_thor <- read_sas(paste0(path2, "cand_thor.sas7bdat"), NULL) %>%  
  zap_formats() %>% zap_labels()

tx_hr <- read_sas(paste0(path2, "tx_hr.sas7bdat"), NULL) %>%  
  zap_formats() %>% zap_labels()

txf_hr <- read_sas(paste0(path2, "txf_hr.sas7bdat"), NULL) %>%  
   zap_formats() %>% zap_labels()
...
29.  Death within 12 months and 18 months post-transplantation (all causes) and date of death
...
tx_list3 <- tx_list2 %>% 
  ...
  mutate(
    ...
    dead_18m = ifelse(dead==1 &
                        (as.Date(rec_death_date) - as.Date(tx_date) < 547), 1, 0)) 
...
# Link with Geo Data
```{r}
#clean donor hospital geographic information
donorhospital_fixed <- donorhospital %>% 
...
donor_geo <- deceasedtodonhosp %>% 
  ...
  left_join(donorhospital_fixed, by = c("DON_HOSP_PROV_NUM" = "PROVIDER_NUM"), keep = FALSE, na_matches = "never") %>% 
...
View(final_sample %>%dplyr::select(DONOR_ID) %>% 
  left_join(deceasedtodonhosp, by = "DONOR_ID", keep = FALSE, na_matches = "never") %>% 
    filter(is.na(DON_HOSP_PROV_NUM))) 
...
final_sample5 <- final_sample4 %>% 
  mutate(
    ...
    # Recipient survival time, administratively censored at 18 months
    #surv_time_18m = ifelse(surv_time > 547 | surv_time == 547, 547, surv_time),
...
#for patients who are censored before 18-month time point, make Day 547 their day of observation, set dead = 1
# 
# final_sample5 <- final_sample5 %>% 
#   mutate(surv_time_18m = ifelse(dead_18m==0 & surv_time_18m <547, 547, surv_time_18m))
# 
# final_sample5 %>% filter(dead_18m == 0 & surv_time_18m<547) %>% View()
...
# write_csv(complete_data_dcd, "complete_data_dcd_1016_ishlt.csv")
write_csv(complete_data_dcd, file = paste0(path2, "complete_data_dcd_1113.csv"))

```

```{r}
save(complete_data_dcd, tx_hr, donor_info, donor, donor_geo, rec_geo, tx_list1, file = paste0(path2, "clean_data_1113_jhlt.RData"))
```
